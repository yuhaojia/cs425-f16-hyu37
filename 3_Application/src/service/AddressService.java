...
public class AddressService {
	...
	public boolean addAddr(int id, Address addr) {
		try {
			...
			pstmt = conn.prepareStatement("insert into associate values(?,?)");
			pstmt.setInt(1, id);
			pstmt.setInt(2, addr.getAddrID());
			pstmt.executeUpdate();
			return true;
	...
	public boolean updateCus(Address addr) {
		try {
			...
			pstmt = conn.prepareStatement("update Address set street=?, city=?, stateName=?, zipcode=?, addrType=? where addrID=?");
