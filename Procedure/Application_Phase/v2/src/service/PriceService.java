...
public class PriceService {
	...
	public boolean addPrice(PricePerState pps) {
		try {
			...
			pstmt = conn.prepareStatement("insert into PriceProductID values(?)");
	...
	public Boolean deletepps(PricePerState pps) {
		...
		try {
			pstmt = conn.prepareStatement("delete from PricePerState where stateName=? and PriceProductID=?");
				pstmt.setString(1, pps.getstateName());
			pstmt.setInt(2, pps.getProductID());
			pstmt.executeUpdate();
			pstmt = conn.prepareStatement("delete from PricePerState where statePrice=?");
